...
public class GeneAlg {
	...
	/**
	 * Function that executes the genetic algorithm
	 * @param value a value, depending on the version
	 * @param version 1 = limit, 2 = borne
	 */
	public void geneticAlgorithm(long value, int version){
		...
		while(evolution){  
			...
			//System.out.println("shortest route = " + this.population.shortestRoute(this.map));
			System.out.println("offspring route = " + offspring.calculLength(this.map));
			System.out.println("a random route = " + this.population.randTr().calculLength(this.map));
			
			
			/*
			//useful way to follow the progression of the algorithm
			if(version == 1){
				long percentage = counter / value;
				if(percentage % 5 == 0){
					System.out.println(percentage + "%");	
				}
			}else if(version == 2){
				long percentage = this.population.lengthShortest(this.map) / value;
				if(percentage % 5 == 0){
					System.out.println(percentage + "%");
				}
			}
		...
		System.out.println("la route la plus courte a pour longueur : ");
